...
type Props = {
  title?: string;
  /** Display name shown near the avatar */
  userName?: string;
};
...
export default function Topbar({
  ...
  const onLogout = async () => {
    try {
      ...
      // ignore network hiccups; we'll still force a nav below
    } finally {
      setMenuOpen(false);
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 items-center gap-3 px-4">
        ...
        <div className="flex items-center gap-3 select-none">
          {/* The SVG lives in /public/logo.svg */}
          <Image
            src="/logo.svg"
            alt="Opinion Elite logo"
            width={36}
            height={36}
            priority
            className="h-9 w-9"
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div
                id="topbar-user-menu"
                role="menu"
                className="absolute right-0 mt-2 w-60 rounded-xl border bg-white shadow-lg z-50 overflow-hidden"
              >
