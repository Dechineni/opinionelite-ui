...
export default function PrescreenPanel({
  ...
  /* -------------------- Sync / EDIT / DELETE handlers for saved list -------------- */
  ...
  const handleSync = async (questionId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/projects/${projectId}/quotas/sync`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            prescreenQuestionId: questionId,
          }),
        }
      );
      ...
      setSuccessMessage("Mapped options synced to Quotas.");
      setSuccessOpen(true);
    } catch (e: any) {
      window.alert(
        e?.message ||
                      ...
                      <tr key={q.id} className={i % 2 ? "bg-slate-50" : "bg-white"}>
                        ...
                        <td className="px-2 py-2">
                          <div className="flex items-center gap-1">
                            <IconButton title="Sync to Quotas" onClick={() => handleSync(q.id)}>
                              {syncingQuestionId === q.id ? (
