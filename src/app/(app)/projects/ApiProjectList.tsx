...
export default function ApiProjectList() {
  ...
  useEffect(() => {
    ...
    const url =
      `/api/api-projects?q=${encodeURIComponent(q)}&page=${page}&pageSize=${pageSize}` +
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="text-lg font-semibold">API Projects</h1>
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="sticky top-0 z-10 bg-slate-800 text-white">
              <tr>
                ...
                <td className="px-4 py-3">SurveyCode</td>
                <td className="px-4 py-3">QuotaId</td>
              ...
              {!loading &&
                rows.map((r, i) => (
                  <tr key={r.id} className={i % 2 ? "bg-slate-50" : "bg-white"}>
                    ...
                    <td className="px-4 py-3">{r.providerType ?? "—"}</td>
                    <td className="px-4 py-3">{r.surveyCode ?? "—"}</td>
                    <td className="px-4 py-3">{r.quotaId ?? "—"}</td>
